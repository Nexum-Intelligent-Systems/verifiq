...
## Developer setup (Phase 1 — schema + LLM adapter + R2 storage adapter)

Phase 1 builds the foundation only: the Convex schema, the provider-agnostic LLM
adapter (Anthropic primary, OpenAI fallback), and the Cloudflare R2 storage
adapter. No agents, no UI, no workflow orchestrator (those are Phase 2+). See
`docs/28-claude-code-phase1-kickoff.md` for the brief and
`docs/29-phase1-completion.md` for what was built.

### Prerequisites

- Node.js ≥ 20
- A Convex account (`npx convex dev` on first run populates `CONVEX_DEPLOYMENT`
  and `NEXT_PUBLIC_CONVEX_URL`)
- API keys for Anthropic and OpenAI
- A Cloudflare R2 bucket + access keys (see `docs/27-stack-decision-storage-and-platform.md`)
...
cd verifiq26
npm install
cp .env.local.example .env.local   # then fill in real values — never commit .env.local
```

### Convex (schema deploy + codegen)

```bash
npx convex dev      # deploys the schema to a preview deployment and writes src/convex/_generated/
# or, without a deployment, just generate types:
...
> Note: `src/types/index.ts` and the smoke test import from
> `src/convex/_generated/`, which is created by `convex dev` / `convex codegen`.
> Run one of those before `npm run typecheck` / `npm test`.

### Test

```bash
npm test            # vitest — runs tests/smoke.test.ts (in-process via convex-test)
```
...
verifiq26/
├─ src/
│  ├─ convex/        # schema.ts, mutations.ts, _generated/ (auto)
│  ├─ llm/           # provider-agnostic LLM adapter (types/config/anthropic/openai/index)
│  ├─ storage/       # storage adapter (types/r2/convex/index)
│  ├─ lib/           # logger, guardrails (locked disclaimer), hash utils
│  ├─ types/         # TypeScript mirror of the schema
│  └─ _legacy_poc/   # archived earlier exploration (not part of the build)
├─ tests/            # smoke.test.ts
└─ docs/29-phase1-completion.md
```
